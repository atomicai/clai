...
from clai.tooling import io, proc, tool
from clai.tunneling import way
...
from haystack.modeling.utils import log_ascii_workers, set_all_seeds
...
class Inferencer:
    ...
    def __init__(
        ...
        self.devices, n_gpu = tool.initialize_device_settings(use_cuda=gpu, multi_gpu=False)

        self._processor = _processor
        self._model = _model
        self._model.eval()
        self.batch_size = batch_size
        self.language = self._model.get_language()
        ...
        if task_type == "embeddings":
            ...
            self._model.prediction_heads = torch.nn.ModuleList([])
            self._model.language_model.extraction_layer = extraction_layer
            self._model.language_model.extraction_strategy = extraction_strategy
        ...
        self._model.connect_heads_with_processor(processor.tasks, require_labels=False)
        set_all_seeds(42)
    ...
    def load(
        ...
        **kwargs,
        ...
        if devices is None:
            devices, n_gpu = tool.initialize_device_settings(use_cuda=gpu, multi_gpu=False)

        name = os.path.basename(model_name_or_path)

        # a) either from local dir
        if os.path.exists(model_name_or_path):
            model = way.WayTunnel.load(load_dir=model_name_or_path, device=devices[0], strict=strict)
            if task_type == "embeddings":
    ...
    def _set_multiprocessing_pool(self, num_processes: Optional[int]) -> None:
        ...
        if num_processes == 0 or num_processes == 1:  # disable multiprocessing
            ...
            io.log_ascii_workers(n=num_processes, logger=logger)
        ...
        * QA (FARM style): [{"questions": ["What is X?"], "text":  "Some context containing the answer"}]
        ...
        if self.process_pool is None:  # multiprocessing disabled (helpful for debugging or using in web frameworks)
            ...
            if multiprocessing_chunksize is None:
                _chunk_size, _ = proc.calc_chunksize(len(dicts))
                multiprocessing_chunksize = _chunk_size

            predictions = self._inference_with_multiprocessing(dicts, return_json, aggregate_preds, multiprocessing_chunksize)
    ...
    def _inference_with_multiprocessing(
        ...
        if self.process_pool is not None:
            results = self.process_pool.imap(
                ...
                tool.grouper(iterable=dicts, n=multiprocessing_chunksize),
                1,
            )
    ...
    def _get_predictions(self, dataset: Dataset, tensor_names: List, baskets):
        ...
        data_loader = proc.NamedDataLoader(
    ...
    def _get_predictions_and_aggregate(self, dataset: Dataset, tensor_names: List, baskets: List[proc.SampleBasket]):
        """
