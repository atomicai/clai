                ...
                if (
                    ...
                    if dev_data_loader is not None:
                        ...
                        if self.early_stopping:
                            ...
                            if save_model:
                                logger.info(
                                    "Saving current best model to {}, eval={}".format(self.early_stopping.save_dir, eval_value)
                                )
        ...
        if self.evaluator_test and self.local_rank in [0, -1]:
            ...
            if test_data_loader is not None:
                evaluator_test = Evaluator(
                    data_loader=test_data_loader, tasks=self.data_silo.processor.tasks, device=self.device, logger=self.logger
                )
        ...
        if self.global_step % self.log_loss_every == 0 and self.local_rank in [-1, 0]:
            if self.local_rank in [-1, 0]:
                ...
                if self.log_learning_rate:
                    self.logger.log_metrics({"learning_rate": self.lr_schedule.get_last_lr()[0]}, step=self.global_step)
        if self.use_amp:
